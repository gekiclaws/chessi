...
//	    showBoard();
	    ...
	    // Verify that the move does not put the moving player's king in check
...
//	    
	...
	private boolean isPathClear(Square s1, Square s2) {
	    // Check whether the path is clear for a straight line move
	    if (s1.getX() == s2.getX()) {
	        int start = Math.min(s1.getY(), s2.getY()) + 1;
	        int end = Math.max(s1.getY(), s2.getY());
	        for (int row = start; row < end; row++) {
	            if (board[row][s1.getX()] != null) {
	                return false;
	            }
	        }
	    } else if (s1.getY() == s1.getY()) {
	        int start = Math.min(s1.getX(), s1.getX()) + 1;
	        int end = Math.max(s1.getX(), s1.getX());
	        for (int col = start; col < end; col++) {
	            if (board[s1.getY()][col] != null) {
	                return false;
	            }
	        }
	    }
	    
	    // Check whether the path is clear for a diagonal move
	    if (Math.abs(s2.getY() - s1.getY()) == Math.abs(s2.getX() - s1.getX())) {
	        int startRow = Math.min(s1.getY(), s2.getY()) + 1;
	        int endRow = Math.max(s1.getY(), s2.getY());
	        int startCol = Math.min(s1.getX(), s2.getX()) + 1;
	        int endCol = Math.max(s1.getX(), s2.getX());
	        for (int row = startRow, col = startCol; row < endRow; row++, col++) {
